...
def get_reward(shapes, params, seed_and_id=None,):
    ...
    if seed_and_id is not None:
        ...
        np.random.seed(seed)
        v = SIGMA * np.random.randn(params.size)
        params_self = params + v
        params_other = params - v
        p_self = params_reshape(shapes, params_self)
        p_other = params_reshape(shapes, params_other)
    else:
        p_self = params_reshape(shapes, params)
    # run episode
    ep_r = 0.
    ep_max = 100
    for episode in range(ep_max):
        s, info = env.reset()
        while True:
            a = get_action(p_self, s)
            if seed_and_id is not None:
                other_a = get_action(p_other, info['other_obs'])
                s, r, done, _, _= env.step(a, other_a)
            else:
            ...
            if done: 
                break
    env.close()
    return [ep_r / ep_max, -ep_r / ep_max]
...
class Trainer:
    ...
    def train(self):
        ...
        for g in range(self.generation):
            ...
            noise_seed = np.random.randint(0, 2 ** 32 - 1, size=self.n_kid, dtype=np.uint32)    # mirrored sampling

            # distribute training in parallel
            jobs = [pool.apply_async(get_reward, (self.net_shapes, self.net_params,
                                                [noise_seed[k_id], k_id], )) for k_id in range(self.n_kid)]
            rewards = np.array([j.get() for j in jobs])
            
            kids_rank = np.argsort(rewards.flatten())[::-1]               # rank kid id by reward

            cumulative_update = np.zeros_like(self.net_params)       # initialize update values
            for ui, k_id in enumerate(kids_rank):
                np.random.seed(noise_seed[k_id // 2])                # reconstruct noise using seed
            ...
            if g % self.eval_freq == 0:
                net_r = get_reward(self.net_shapes, self.net_params, None,)[0]
                mar = net_r if mar is None else 0.9 * mar + 0.1 * net_r       # moving average reward
                print(
                    'Gen: ', g,
                    '| Net_R: %.1f' % net_r, 
                    '| Avg_R: %.1f' % mar,
                    '| Kid_avg_R: %.1f' % rewards[:, 0].mean(),
                    '| Gen_T: %.2f' % (time.time() - t0),)
                
                with open('record.txt', 'a+') as f:
                    f.write(f'Gen: {g} | Net_R: {net_r} | Avg_R: {mar} | Kid_avg_R: {rewards[:, 0].mean()} | Gen_T: {(time.time() - t0)}\n')
